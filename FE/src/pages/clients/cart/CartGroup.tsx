...
export const CartGroup = ({ cart }: CartGroupProps) => {
				...
				<div className="space-y-2">
					{cart?.items?.map((item) => {
						const attributeAlreadyExists = cart?.items?.reduce(
							(acc, p) => {
								if (p._id !== item._id) {
									const colorId = (p?.attribute?.colorId as any)?._id;
									const sizeId = (p?.attribute?.sizeId as any)?._id;
									if (
										colorId &&
										colorId !== (item?.attribute?.colorId as any)?._id &&
									...
									if (
										sizeId &&
										sizeId !== (item?.attribute?.sizeId as any)?._id &&
						...
						return (
							...
							>
								...
								<motion.div
									...
									dragElastic={0.1}
									onDragEnd={(_, info) =>
										handleDragEnd(item._id as string, info)
									}
									animate={{
